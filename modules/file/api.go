...
// Route 路由
func (f *File) Route(r *mshttp.MSHttp) {
	api := r.Group("/v1/file")
	{ // 文件上传
...
func (f *File) makeImageCompose(c *mshttp.Context) {
...
// 获取上传文件地址
func (f *File) getFilePath(c *mshttp.Context) {
...
// 上传文件
func (f *File) uploadFile(c *mshttp.Context) {
	uploadPath := c.Query("path")
	fileType := c.Query("type")
...
// 获取文件
func (f *File) getFile(c *mshttp.Context) {
	ph := c.Param("path")
	if ph == "" {
