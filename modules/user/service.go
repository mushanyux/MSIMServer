...
import (
	...
	"github.com/mushanyux/MSIMServer/modules/source"
	"go.uber.org/zap"
)
...
func (s *Service) GetUserDetail(uid string, loginUID string) (*UserDetailResp, error) {
	...
	if friend != nil && friend.IsDeleted == 0 {
		follow = 1
		//查询加好友来源
		sourceFrom = source.GetSource(friend.SourceVercode)
...
type UserDetailResp struct {
	...
	Code                string            `json:"code"`                   //加好友所需vercode TODO: code不再使用 使用Vercode
