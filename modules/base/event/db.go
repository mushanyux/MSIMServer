...
func (d *DB) QueryAllWait(limit uint64) ([]*Model, error) {
	var models []*Model
	_, err := d.session.Select("*").From("event").Where("status=? and created_at<?", msevent.Wait.Int(), util.ToyyyyMMddHHmmss(time.Now().Add(-time.Second*60))).Limit(limit).Load(&models)
	return models, err
}
